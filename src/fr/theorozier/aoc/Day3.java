...
public class Day3 {
	...
	private enum Dir {
		...
		private static final char[] codes = new char[values().length];
		...
		static Dir getDirection(char code) {
	...
	private static class Pos {
		...
		Pos(int x, int y, int step) {
			...
			this.x = x;
			this.y = y;
			this.dist = computeManhattanDist();
			this.step = step;
			
		}
		
		Pos add(Dir dir) {
			return new Pos(this.x + dir.rx, this.y + dir.ry, this.step + 1);
		}
		
		int computeManhattanDist() {
	...
	private static List<Action> parseActions(String input) {
		...
		for (String rawAction : input.split(",")) {
		
			dirCode = rawAction.charAt(0);
			dir = Dir.getDirection(dirCode);
			
			if (dir != null) {
				
				try {
					
					rawLength = rawAction.substring(1);
					length = Integer.parseInt(rawLength);
					
					actions.add(new Action(dir, length));
	...
	private static Map<Pos, Pos> computePositions(List<Action> actions) {
		
		Map<Pos, Pos> ret = new HashMap<>();
		Pos pos = new Pos(0, 0, 0);
		
		for (Action action : actions) {
			for (int i = 0; i < action.length; i++) {
			
				pos = pos.add(action.dir);
				ret.put(pos, pos);
	...
	private static List<Pos> computeIntersections(Map<Pos, Pos> map1, Map<Pos, Pos> map2) {
		
		List<Pos> intersects = new ArrayList<>();
		Pos pos;
		
		for (Pos map1pos : map1.keySet()) {
			if ((pos = map2.get(map1pos)) != null) {
				intersects.add(new Pos(map1pos.x, map1pos.y, map1pos.step + pos.step));
			}
		}
	...
	public static void main(String[] args) {
		...
		Map<Pos, Pos> map1 = computePositions(parseActions(input1));
		Map<Pos, Pos> map2 = computePositions(parseActions(input2));
		
		System.out.println("Map1 : " + map1.size());
		System.out.println("Map2 : " + map2.size());
		
		List<Pos> intersections = computeIntersections(map1, map2);
		intersections.sort((p1, p2) -> p2.step - p1.step);
		
		System.out.println("Computing finished :");
		
		for (Pos intersect : intersections)
			System.out.println("- " + intersect + " (dist: " + intersect.dist + ", steps: " + intersect.step + ")");
